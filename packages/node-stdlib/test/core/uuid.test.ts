...
describe("core/uuid.ts", () => {
  describe("nil uuid", () => {
    it("treats all nil uuids as equal", () => {
      ...
      const nilInstance = UUID.fromString("00000000-0000-0000-0000-000000000000");
  ...
  describe("fromString()", () => {
    it("returns undefined when the string is an invalid uuid", () => {
      expect(UUID.fromString("abcde")).toBeUndefined();
      expect(UUID.fromString("1h4f6d02-1146-48a8-b0ad-f562220303de")).toBeUndefined();
    });

    it("returns undefined if the uuid has an invalid version", () => {
      expect(UUID.fromString("1d4f6d02-1146-78a8-b0ad-f562220303de")).toBeUndefined();
    });
  });
  ...
  describe("v4", () => {
    ...
    it("creates a valid v4 uuid", () => {
      ...
      expect(uuid.version()).toBe(UUID.Version.v4);
      expect(uuid.variant()).toBe(UUID.Variant.rfc4122);
    });
    ...
    describe("v3 / v5", () => {
      const namespace = UUID.fromString("1d4f6d02-1146-48a8-b0ad-f562220303de")!;

      it("creates a valid v3 uuid", () => {
        const uuid = UUID.v3(namespace, "hello");
        expect(uuid.toString()).toBe("2dcc63e2-4ad4-30e2-98a9-609940113153");
      });

      it("creates a valid v3 uuid", () => {
        const uuid = UUID.v5(namespace, "hello");
        expect(uuid.toString()).toBe("0b6de54f-2342-5355-aa90-44957e68d403");
      });

      it("creates the same uuid for the same namespace and name", () => {
        const uuid1 = UUID.v5(namespace, "always the same");
        const uuid2 = UUID.v5(namespace, "always the same");
